import { DefaultMessageConfig, LoadingHandler, MJMessage, MessageConfig, MessageConfigParam, MidjourneyConfig } from './interfaces'
...
export class MidjourneyMessage {
	...
	async FilterMessages(prompt: string, loading?: LoadingHandler, options?: string, index?: number) {
		...
		for (let i = 0; i < data.length; i++) {
			...
			if (item.author.id === '936929561302675456' && item.content.includes(`${prompt}`)) {
				...
				if (options && !(item.content.includes(options) || (options === 'Upscaled' && item.content.includes(`Image #${index}`)))) {
					this.log('no options')
					continue
				}
	...
	async RetrieveMessages(limit = this.config.Limit) {
		...
		const response = await fetch(`https://discord.com/api/v10/channels/${this.config.ChannelId}/messages?limit=${limit}`, {
			headers: headers,
		})
		if (!response.ok) {
