...
import { MessageConfig, MessageConfigParam, DefaultMessageConfig, WaitMjEvent, MJMessage, LoadingHandler, WsEventMsg, ImageEventType } from './interfaces'
...
export class WsMessage {
	...
	private async verifyHumanApi(custom_id: string, message_id: string, nonce?: string) {
		const payload = {
			type: 3,
			nonce,
	...
	private filterMessages(MJmsg: MJMessage) {
		...
		const index = this.waitMjEvent.findIndex((e) => this.content2prompt(e.prompt) === this.content2prompt(MJmsg.content))
		if (index < 0) {
	...
	onceImage(type: ImageEventType, nonce: string, callback: (data: WsEventMsg) => void) {
	...
	async waitMessage(type: ImageEventType, nonce: string, loading?: LoadingHandler) {
